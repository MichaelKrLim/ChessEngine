...
int main()
{
    ...
    const auto count_ancestor_nodes = [&move_generator](this auto&& rec, int depth, const Board& board, int num_branches) -> int
    {
        if(depth == 0)
            return num_branches;
        
        for(const auto& all_moves : move_generator.get(board))
        {
    ...
    const std::array<std::unordered_map<Position, std::vector<Position>>, number_of_piece_types> initial_moves = move_generator.get(base_position);
