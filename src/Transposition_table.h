...
namespace engine
{
    ...
    template <std::size_t table_size_log2>
    class Transposition_table
    {
        public:
        
        [[nodiscard]] Transposition_data& operator[](const State& state)
        {
            const auto index = zobrist::hash(state) % data.size();
            return data[index].value();
        }

        [[nodiscard]] std::optional<Transposition_data>& find(const State& state)
        {
            const auto index = zobrist::hash(state) % data.size();
            return data[index];
        }
        ...
        std::array<std::optional<Transposition_data>, (1ULL<<(table_size_log2-1))-1> data{std::nullopt};
    };
}
