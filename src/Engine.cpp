...
std::expected<Search_results, Engine::search_stopped> Engine::generate_best_move(std::stop_token stop_token, const Search_options& search_options) noexcept
{
	...
	{
		return std::ranges::count(state_.repetition_history | std::views::reverse | std::views::take(state_.half_move_clock), state_.zobrist_hash)>=3;
	};
	...
	const auto quiescence_search = [&](this auto&& rec, double alpha, double beta, unsigned& extended_depth, const unsigned& current_extended_depth, unsigned& nodes, const unsigned depth) -> double
	{
		...
		for(const auto& move : generate_moves<Moves_type::noisy>(state_))
		{
			if(std::optional<Piece> piece_to_capture=state_.piece_at(move.destination_square(), other_side(state_.side_to_move)); piece_to_capture && stand_pat+chess_data::piece_values[Side::white][piece_to_capture.value()]<=alpha)
				continue;
			state_.make(move);
	...
	const auto nega_max = [&](this auto&& rec, const unsigned remaining_depth, unsigned& extended_depth, unsigned current_extended_depth, unsigned number_of_checks_in_current_line, unsigned& nodes, const unsigned& depth, Fixed_capacity_vector<Move, 256>& pv, double alpha = -std::numeric_limits<int>::infinity(), double beta = std::numeric_limits<double>::infinity())
	{
		...
		const auto cache_result=transposition_table_[state_.zobrist_hash];
		...
		{
			const auto lhs_victim_value = std::to_underlying(state_.piece_at(lhs.destination_square(), other_side(state_.side_to_move)).value());
			const auto rhs_victim_value = std::to_underlying(state_.piece_at(rhs.destination_square(), other_side(state_.side_to_move)).value());
			if(lhs_victim_value < rhs_victim_value)
				return false;
			if(lhs_victim_value > rhs_victim_value)
				return true;

			const auto lhs_attacker_value = std::to_underlying(state_.piece_at(lhs.from_square(), state_.side_to_move).value());
			const auto rhs_attacker_value = std::to_underlying(state_.piece_at(rhs.from_square(), state_.side_to_move).value());
		...
		std::ranges::sort(all_legal_moves, [&](const Move& lhs, const Move& rhs)
		{
			...
			const bool lhs_is_capture = !is_free(lhs.destination_square(), state_.sides[other_side(state_.side_to_move)].occupied_squares()),
					   rhs_is_capture = !is_free(rhs.destination_square(), state_.sides[other_side(state_.side_to_move)].occupied_squares());
		...
		const bool in_check{state_.in_check()};
		...
		{
			if(state_.is_square_attacked(state_.sides[state_.side_to_move].pieces[Piece::king].lsb_square()))
		...
		if(std::abs(alpha-beta)>1)
		{
			transposition_table_.insert(Transposition_data
			{
				...
				.eval=best_score,
				.zobrist_hash=state_.zobrist_hash,
