...
void Board::make(const Move& move) noexcept
{
	...
	else if(is_castling)
	{
		...
		side.pieces[static_cast<std::uint8_t>(Piece::king)] ^= (1ULL << to_index(destination_square)) & (1ULL << to_index(from_square));
		side.pieces[static_cast<std::uint8_t>(Piece::rook)] ^= (1ULL << to_index(rook_origin_square) & (1ULL << to_index(rook_destination_square)));
